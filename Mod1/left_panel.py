...
class LeftPanel:
    def __init__(self, root, frame):
        ...
        # label for instruction bit
        Label(self.frame, text="Instructions:").grid(row=0, column=0,
                                                     padx=10, pady=2)
        self.instruct = Label(self.frame, text="..instruction label left...")
        self.instruct.grid(row=1, column=0, padx=10, pady=2)

        try:
            self.instruction_image_ex = PhotoImage(file="image.jpeg")
            Label(self.frame, image=self.instruction_image_ex).grid(row=2,
                                                                    column=0,
                                                                    padx=10,
                                                                    pady=2)
        except:
            print(f"left panel instruction image is not found")
