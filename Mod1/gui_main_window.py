...
class MainWindow:
    ...
    def __init__(self):
        ...
        # runs the event loop to monitor events/changes in the GUI window
        self.root.mainloop()

    def threading(self):
        """
        This function is in charge of being able to run multiple threads (
        tasks and/or function calls) at the same time. It allows python to
        execute other code while the program is waiting to complete another
        task.

        :return: separate task requiring execution.
        """
        thread_one = Thread(target=self.start_clock)
        thread_one.start()
    ...
    def sound_alarm(timer):
        ...
        while True:
            ...
            """
            Get the current time (date and time) and use string conversion to
            format it to display as H:M:S.
            """
            current_time = datetime.datetime.now().strftime("%H:%M%S")
            print(f"current time: {current_time} alarm set: {set_alarm}")
            ...
            if current_time == set_alarm:
                ...
                print(f"The alarm sounded. It is: {current_time}")
                winsound.PlaySound("sound.wav", winsound.SND_ASYNC)
                break
    """
    Here we can use the global level to declare the attributes and needed 
    elements for the GUI window. 
    """

    """
    Sets the hour variable to a string and makes it to automatically populate 
    with 00 when it is displayed.
    """
    ...
    Button(self.root, text="Set Alarm", font=("Helvetica 15 bold"),
